...
;(function ($, window) {
	 ...
	 * @param hint [boolean] <false> "Add hint to input with first matched name, correct styles should be installed"
	 ...
	 * @param customLabel [boolean] <false> "The name of object's property which will be used as a name"
            ...
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                    if (response.length < limit) {
                        switch (i) {
                            case 0:
                                if (source[item].name.toUpperCase().search(query) === 0) {
                            ...
                            case 1:
                                if (source[item].name.toUpperCase().search(query) !== -1) {
                                    response.push(source[item]);
                                    delete source[item];
                                }
                            break;
    ...
    function _buildList(list, data) {
        ...
        for (var item = 0, count = list.length; item < count; item++) {
            ...
            var name = (data.customLabel && list[item][data.customLabel]) ? list[item][data.customLabel] : list[item].name;

            var clear = name;

            name = data.highlightMatches ? name.replace(highlightReg, "<strong>$&</strong>") : name;

            var value = (data.customValue && list[item][data.customValue]) ? list[item][data.customValue] : list[item].value;

            // Apply custom template
            if (data.template) {
                var template = data.template.replace(/({{ name }})/gi, name);
            ...
            if (value) {
                menu += '<li data-value="'+value+'" data-name="'+clear+'" class="'+classes.join(' ')+'">'+name+'</li>';
            } else {
                menu += '<li data-name="'+clear+'" class="'+classes.join(' ')+'">'+name+'</li>';
            }
        }
        ...
        if (list.length && data.hint) {
            ...
            if (hint && (data.query !== list[0].name)) {
    ...
    function _onKeydownHelper(e) {
        ...
        if (code === 40 || code === 38 ) {
            ...
            if (data.hint && data.hintText && data.$autocompleter.find('.autocompleter-hint').hasClass('autocompleter-hint-show')) {
                ...
                var hintOrigin = data.$autocompleter.find(".autocompleter-item").length ? data.$autocompleter.find(".autocompleter-item").eq(0).attr('data-name') : false;
                if (hintOrigin) {
    ...
    function _setValue(data) {
        if (data.$selected) {
            ...
            var value = data.$selected.attr('data-value') ? data.$selected.attr('data-value') : data.$selected.attr('data-name');
            data.$node.val(value);
        } else {
            if (data.hintText && !data.$autocompleter.find('.autocompleter-hint').hasClass('autocompleter-hint-show')) {
